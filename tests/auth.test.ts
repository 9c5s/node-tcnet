...
describe("pending状態でのcmd=1再受信 (初回認証中の反応型プロトコル)", () => {
    ...
    it("pending中のtoken変化時は旧sendAuthSequenceが新世代のstateを破壊しない (stale promise race)", async () => {
        // 本物の sendAuthSequence を通した stale Promise race 回帰テスト。
        // 旧世代の 50ms wait 中に新 token を注入し、prepareAuthPayload が null 返却
        // しても resetAuthSession が呼ばれず、新世代の state が維持されることを検証する。
        vi.useFakeTimers();
        try {
            const client = new AuthSequenceTestClient();
            client.setBridgeIsWindows(false);
            ...
            // 初回 cmd=1 (token A): handleInitialAuthRequest → 本物の sendAuthSequence 起動
            client.callHandleAuth(createAppDataPacket(1, 0xaaaaaaaa));
            expect(client.getSessionToken()).toBe(0xaaaaaaaa);
            expect(client.authenticationState).toBe("pending");

            // 50ms wait の半分まで進める (旧 sendAuthSequence は依然 wait 中)
            ...
            // 残り 25ms 進めて旧世代の 50ms wait を完了させる
            // 旧 sendAuthSequence は prepareAuthPayload で tokenBeforePing !== sessionToken
            // により null 返却し、expectedToken ガードで resetAuthSession を呼ばずに return
            await vi.advanceTimersByTimeAsync(25);
            await flushAsync(10);

            // 新世代の state が維持されていることを検証
            expect(client.getSessionToken()).toBe(0xbbbbbbbb);
            expect(client.authenticationState).toBe("pending");
        } finally {
            vi.useRealTimers();
