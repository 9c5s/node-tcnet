...
各CUEエントリの byte+0 (CUE Type) は Memory Cue と Hot Cue A-H を識別する値が入る。仕様書には値の意味が明記されていないが、実機 (BRIDGE + CDJ-3000) では以下の対応で送出される。
...
アプリ側で Hot Cue A-H ラベルを表示したい場合は `String.fromCharCode(64 + cue.type)` で取得できる。
