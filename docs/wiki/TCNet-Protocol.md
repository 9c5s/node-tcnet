...
TCNetはエンターテイメント業界向けに設計されたオープンな通信プロトコルである。デバイスやソフトウェア間でリアルタイムのタイムコードとメタデータをUDP経由で共有する。
...
MessageType=200のData内のサブタイプである。
...
LM TimeとLC Timeが同じオフセット48と記載されているが、正しくはLM=48, LC=52である。Beat Markerも同様にLM=94, LC=95である。
